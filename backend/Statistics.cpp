...
// vector<double> extract_amount(vector<string>& transaction_data){
//     std::regex amountPattern(R"(Amount:\s([\d\.]+)\sINR)");
//     std::vector<double> amounts;

//     for (const auto& transaction : transaction_data) {
//         std::smatch match;
//         if (std::regex_search(transaction, match, amountPattern)) {
//             double amount = std::stod(match[1].str());  // Convert matched string to double
...
// KMP preprocessing to create the "longest prefix suffix" (LPS) array
...
// KMP search function to find the start index of "Amount:"
...
unordered_map<string,double> Transaction_history::monthlysummary(int month,int year,string username){
    ...
    // if((!month) || (!year)){
    //     pair<int,int> month_year=getCurrentMonthYear();
    //     month=month_year.first;
    //     year=month_year.second;
    // }
    std::string str_month="";
    if(month<=9) str_month ="0"+to_string(month);
    else str_month=to_string(month);
    ...
    if(transaction_data[0]=="No Transactions Available") return stats;
