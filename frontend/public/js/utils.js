...
anomaly_detector_url = "http://127.0.0.1:5001/detect_anomaly"
const detect_anomaly = async (event) => {
  event.preventDefault();
  const formData = new FormData(event.target);
  const data = Object.fromEntries(formData.entries());

  try {
    const response = await fetch(anomaly_detector_url, {
      method: "POST",
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (response.status === 500) {
      console.log("Server error in anomaly detector");
      return { error: "server error in anomaly detector" };
    } else if (response.status === 200) {
      return await response.json();
    }
  } catch (error) {
    console.error('Error:', error);
    showError('An error occurred in anomaly detection. Please try again. Details: ' + error.message);
  }
};
      ...
      if (data.transactions.length === 0) {
        ...
        if (!showingall) {
          const transactionsToDisplay = data.transactions.length > 5
            ? data.transactions.slice(0,6) // Get the last 6 transactions
